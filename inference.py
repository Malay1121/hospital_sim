...
- LOCAL_IMAGE_NAME / IMAGE_NAME (default: hospital_scheduler-env:latest)
"""
...
try:
    ...
    # When installed as a package (generated by openenv init)
    from my_env import HospitalSchedulerEnv, HospitalAction  # type: ignore
...
IMAGE_NAME = os.getenv("LOCAL_IMAGE_NAME") or os.getenv("IMAGE_NAME") or "hospital_scheduler-env:latest"
...
async def _run_task(task: str) -> Tuple[bool, int, float, List[float]]:
    llm = OpenAI(base_url=API_BASE_URL, api_key=API_KEY or "NO_KEY")

    # Start a fresh container per task (most robust for evaluation).
